...
const NotificationFilter = ({
  ...
  const dropdownTriggerRefs = useRef([]); // Ref for the trigger div
  const dropdownContentRefs = useRef([]); // Ref for the dropdown content
  const [dropdownPositions, setDropdownPositions] = useState({}); // To store positions for portal
  const containerRef = useRef(null); // Ref for the whole NotificationFilter container

  const singleSelectFilters = ['Created On', 'Due Date', 'Status', 'Next Step Due Date'];

  // Handle clicks outside to close dropdowns
  const handleClickOutside = useCallback((event) => {
    // If no dropdown is open, or if the event target is null, do nothing
    if (openDropdownIdx === null || !event.target) {
      return;
    }

    const triggerEl = dropdownTriggerRefs.current[openDropdownIdx];
    const contentEl = dropdownContentRefs.current[openDropdownIdx];

    // If the click target is the trigger element or inside it, or if the click is inside the dropdown content, DO NOT close.
    // This allows interaction with the dropdown itself.
    if (
    ...
    // If the click is outside both the trigger and the content, then close the dropdown.
  ...
  const updateDropdownPosition = useCallback((idx) => {
    const triggerEl = dropdownTriggerRefs.current[idx];
    if (triggerEl) {
      const rect = triggerEl.getBoundingClientRect();
      setDropdownPositions(prev => ({
        ...prev,
        [idx]: {
          top: rect.bottom + window.scrollY + 4, // 4px margin below trigger
  ...
  useEffect(() => {
    if (openDropdownIdx !== null) {
      updateDropdownPosition(openDropdownIdx);
      // Recalculate position on scroll/resize for better responsiveness
      const handleScrollOrResize = () => updateDropdownPosition(openDropdownIdx);
  ...
  }, [openDropdownIdx, updateDropdownPosition]);
  ...
  const renderAttributes = (filter, idx, attributes, isMultiSelect) => {
    ...
    if (isMultiSelect || filter.filterBy === 'Issue Types') {
      ...
      const dropdownContent = openDropdownIdx === idx && (
        <div
          className={`absolute z-50 max-h-52 overflow-y-auto border border-gray-300 rounded-md bg-white shadow`}
          style={{
            top: dropdownPositions[idx]?.top,
            left: dropdownPositions[idx]?.left,
            width: dropdownPositions[idx]?.width,
          }}
          ref={el => (dropdownContentRefs.current[idx] = el)} // Assign ref to the dropdown content
        >
          {/* Sticky Search Box */}
      ...
      return (
        <>
          ...
          <div
            onClick={() => {
              ...
              if (openDropdownIdx !== idx) {
                setTimeout(() => updateDropdownPosition(idx), 0);
              }
            }}
            ...
            ref={el => (dropdownTriggerRefs.current[idx] = el)} // Assign ref to the trigger
          >
          ...
          {/* Render dropdown content into a portal */}
          {openDropdownIdx === idx && ReactDOM.createPortal(
            dropdownContent,
            document.body // Portals the dropdown content directly to the body
          )}
        </>
      );
    }

    // Single select fallback (remains unchanged as it's a native select element)
    return (
      <select
      ...
      {filters.map((filter, idx) => {
        ...
        return (
          <div
            ...
            key={idx}
            // The ref is now on the trigger div inside renderAttributes
          >
            {/* Filter By Dropdown */}
