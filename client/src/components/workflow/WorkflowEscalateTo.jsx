...
const WorkflowEscalateTo = ({
  ...
  const filteredCompanies = Array.isArray(companies)
    ? companies.filter(
        (c) =>
          ...
          !selectedCompanies.some((sc) => sc.id === c.id) // <--- Changed: Check if company object already exists by ID
      )
    : [];
  ...
  const toggleCompanySelection = (company) => {
    setSelectedCompanies((prev) =>
      prev.some((c) => c.id === company.id) // <--- Changed: Check for company object by ID
        ? prev.filter((c) => c.id !== company.id) // <--- Changed: Filter out company object by ID
        : [...prev, company] // <--- Changed: Add the full company object
    );
    setSearchCompany("");
  };
  ...
  // getSelectedName now expects the full item for companies and users, or the ID for roles
  const getSelectedName = (item, list, type) => {
    if (type === "role") {
      const roleItem = list.find((r) => r.roleGroupId === item);
      return roleItem ? roleItem.name : "Unknown";
    } else if (type === "company") {
      // 'item' for company is already the object from selectedCompanies
      return item ? item.name : "Unknown";
    } else if (type === "user") {
      // 'item' for user is already the object from selectedUsers
      return item ? (item.name || `${item.firstName || ""} ${item.lastName || ""}`) : "Unknown";
    }
    return "Unknown";
  };
        ...
        {selectedItems.length > 0 ? (
          <div className="flex flex-nowrap gap-1 w-max flex-shrink-0">
            {selectedItems.map((item) => (
              ...
              >
                {getSelectedName(
                  item, // <--- Pass the full item for company/user, ID for role
                  type === "role" ? roles : type === "company" ? companies : users,
                  type // <--- Pass the type to getSelectedName
                )}
                <button
                  type="button"
                  onClick={(e) => {
                    ...
                    toggleSelection(
                      type === "role"
                        ? { roleGroupId: item } // Pass an object for role removal to match toggleRoleSelection
                        : item // <--- Pass the full item for user/company removal
                    );
                  }}
        ...
        >
          ...
          {filteredList.length > 0 ? (
            filteredList.map((item) => (
              ...
              >
                <span className="text-sm text-gray-700">{item.name || `${item.firstName || ""} ${item.lastName || ""}`}</span>
              </button>
            ))
          ) : (
