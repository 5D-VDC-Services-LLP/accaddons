...
//   WorkflowHeader,
//   WorkflowTitle,
//   WorkflowFilters, // This component handles user, company, role selection
...
//   // States for Escalation (WorkflowFilters component)
//   const [searchUser, setSearchUser] = useState('');
//   const [searchCompany, setSearchCompany] = useState('');
//   const [searchRole, setSearchRole] = useState('');
//   const [selectedUsers, setSelectedUsers] = useState([]); // Stores selected user objects {autodeskId, email}
//   const [selectedCompanies, setSelectedCompanies] = useState([]); // Stores selected company objects {id, name}
//   const [selectedRoles, setSelectedRoles] = useState([]); // Stores selected role strings
...
//           setSelectedDays(data.frequency || ['mon', 'tue', 'wed', 'thu', 'fri']); // frequency is top-level
//           setIsWhatsAppSelected(data.channels?.includes('whatsapp') || false);
//           setIsEmailSelected(data.channels?.includes('email') || false);

//           // Populate escalate_to fields
          
//           setSelectedUsers(data.escalate_to?.users || []);

//           // For companies, assuming fetched data has full company objects, or just IDs if that's what's stored
//           // If only IDs are stored, you might need to map them to full objects from `companies` state later
//           setSelectedCompanies(data.escalate_to?.company || []); // Assuming company stores IDs
//           setSelectedRoles(data.escalate_to?.roles || []);
//           console.log("Selected Roles:", data.escalate_to?.roles || []);

//           // Populate filters
//           // If filters are complex objects, ensure deep copy or proper mapping
...
//       // Set default title for new workflow
//       setTitle('New Workflow Configuration');
//     }
//   }, [workflow_id]); // Rerun when workflow_id changes

//   // 2. Fetch users, companies, roles data for dropdowns (runs once projectId is available)
//   useEffect(() => {
//     const fetchDropdownData = async () => {
//       // Only set loading/error for dropdown data, not overall loading if already handled by workflow_id fetch
//       if (!workflow_id) setLoading(true); // Only set overall loading if not already loading for workflow_id
//       setError(null);
...
//         // Assuming roles are nested within usersData or fetched separately
//         // If roles are fetched from usersData.data.roles, ensure usersData.data exists
...
//       } finally {
//         if (!workflow_id) setLoading(false); // Only unset overall loading if not in workflow_id fetch
//       }
//     };
...
//       fetchDropdownData();
//     }
//   }, [projectId, workflow_id]); // Dependency on projectId and workflow_id (to avoid re-fetching if workflow_id is loading)


//   // 3. Fetch module-specific filters (runs once projectId and moduleType are available)
//   useEffect(() => {
...
//         if (!workflow_id || (workflow_id && filters.length === 0 && data?.data)) {
...
//       fetchModuleFilters();
//     }
//   }, [projectId, moduleType, workflow_id]); // Added workflow_id to dependencies to re-evaluate default filters if needed
...
//         company: selectedCompanies.map(c => c.id || c), // Assuming selectedCompanies might be objects or just IDs
//         roles: selectedRoles
//       },
//       filters: filters,
...
//       alert(`Workflow ${isEditMode ? 'updated' : 'created'} successfully!`); // Use alert for simplicity, replace with custom modal
//       navigate(-1); // Go back to the previous page (Dashboard)

//     } catch (err) {
//       console.error("Error saving workflow:", err);
//       alert(`Failed to ${isEditMode ? 'update' : 'save'} workflow: ${err.message}`); // Using alert for simplicity
//     }
//   };
